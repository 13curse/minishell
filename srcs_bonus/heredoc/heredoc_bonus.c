...
/*   Updated: 2025/06/13 11:25:01 by van              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	heredoc_final_part(t_minishell *shell, int i)
{
	if (shell->last_redir_in[shell->cmd_id - 1][i] > 0)
		close(shell->last_redir_in[shell->cmd_id - 1][i]);
	else if (shell->last_redir_in[shell->cmd_id - 1][i] < -1)
		return (0);
	shell->last_redir_in[shell->cmd_id - 1][i]
		= shell->heredocs->fds[shell->heredocs->i];
	return (0);
}
...
int	heredoc(t_minishell *shell, char *limiter, int i)
{
	...
	return (heredoc_final_part(shell, i));
}
